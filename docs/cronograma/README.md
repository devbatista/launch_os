...
| 2.3 | `Client` (`find_or_create_by` email) e `Order` com estados pending/paid/failed/refunded/disputed; services `Orders::*` com lock | [07](../specs/07-checkout-paypal.md), [03](../specs/03-modelo-de-dados.md) | 6 | 2.2 | 🟦 | | Transições (MarkPaid/Failed/Refunded/Disputed/ResolveDispute) entregues em 2.1/2.2; faltam token (2.4) e email (2.6) |
| 2.4 | `DownloadToken`, Thank You, `GET /download/:token` com URL assinada, expiração e contagem | [08](../specs/08-entrega-download-tokens.md) | 6 | 2.3 | 🟦 | | 18/09: token, Thank You (3 estados), download com URL assinada de 5 min testado no MinIO; falta só o download em produção (M2) |
| 2.5 | Recuperação de acesso (`/access/recover`) com resposta neutra, rate limit e honeypot | [08](../specs/08-entrega-download-tokens.md) | 3 | 2.4 | ⬜ | | |
| 2.6 | ⚙️ Jobs em background (**Sidekiq**), `OrderMailer` (acesso, reenvio, reembolso) via **SES**, `MessageLog` | [09](../specs/09-notificacoes-email-whatsapp.md) | 6 | 2.4, 0.4 | ⬜ | | |
| 2.7 | WhatsApp via Twilio: campo de telefone com opt-in na LP, gravação no `Client`, envio pós-webhook, callbacks de status e inbound (STOP), validação de assinatura | [09](../specs/09-notificacoes-email-whatsapp.md) | 8 | 2.6, 0.3 (Sandbox) | ⬜ | | Sem sender aprovado: concluir no Sandbox da Twilio; troca para o número real é só ENV |
| 2.8 | Admin de pedidos e clientes: listas, detalhe, reenvio por canal, regenerar/revogar token, disputas, webhook events | [11](../specs/11-admin-pedidos-clientes-dashboard.md) | 5 | 2.6, 2.7 | ⬜ | | Admin de clientes pode ir para Fase 5 se faltar tempo |
