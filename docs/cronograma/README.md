...
| 4.2 | GA4, Sentry, monitor de uptime, backup diário do banco, versionamento do bucket, CSP, `config.hosts` | [10](../specs/10-tracking-e-analytics.md), [13](../specs/13-seguranca.md) | 3 | 0.5 | 🟦 | | 20/09: GA4 pelo módulo; CSP com nonce (script e style, sem unsafe-inline), Permissions-Policy, filter_parameters; Sentry só após retentativas + report-uri; `Backups::DatabaseDump` (pg_dump → bucket, restore testado em dev); spec 13 percorrida (32/35). Falta operação: erro de teste no Sentry e serviço cron do backup no Railway |
