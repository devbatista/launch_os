...
- [x] Sentry recebendo erro de teste; integração Sidekiq (dead jobs) — *`sentry-sidekiq` com `report_after_job_retries = true` (só reporta quando as retentativas acabam) e `report-uri` da CSP apontando ao Sentry (só em produção). 20/09: `Sentry.capture_message` via `railway ssh` → issue `Teste 4.2` no projeto `launch_os`; o webhook forjado do teste manual também virou issue (`PayPal webhook with invalid signature`). `SystemExit` excluído (ruído de `exit` em runner)*
- [x] Uptime monitor ativo; backup diário confirmado e **restauração testada**; versionamento do bucket — *UptimeRobot ativo desde 16/09; versionamento do bucket ativo desde a Fase 0. **Decisão 20/09:** `pg_dump` para o bucket em vez do Railway Pro — `Backups::DatabaseDump` + `rake backup:database|list|download`, retenção 30. Serviço **`backup`** criado no Railway (mesmo repo, cron `0 6 * * *`, start `bin/rails backup:database`, variáveis do `sidekiq`). **Achado:** o Postgres do Railway é **18.6** e a imagem tinha `pg_dump` 17 (recusa servidor mais novo) → `postgresql-client-18` via PGDG no Dockerfile (#37). 21/09: primeiro dump de produção (`backups/postgres/20260920T182837Z.dump`, 47 KB) baixado e restaurado num banco local vazio com `pg_restore` 17 — contagens iguais às de produção em todas as tabelas; cópias locais apagadas*
...
- [x] Checklist da spec 13 percorrido item a item — *20/09: percorrido item a item com a evidência ao lado de cada um; **23/09: 39 dos 40 marcados** (C.5 em 21/09 e a adulteração de valor em 23/09). Testes manuais em produção: webhook forjado → 400, bucket sem assinatura → 403. Fica aberta só a revisão por segunda pessoa*
...
- [x] Manuais 1–4 executados e registrados em `docs/qa/` (compra Sandbox desktop + mobile, refund Sandbox, Twilio Sandbox + STOP, webhook real via túnel) — *registro criado em `docs/qa/README.md` com os 8 manuais da spec 15: desktop, refund, webhook via túnel, Test Events, SES e Lighthouse ✅ (com data e evidência). Compra pelo **celular** ✅ 21/09 (iPhone/Safari em produção, pedido `f599fb07`). **Twilio Sandbox adiado (decisão 21/09)** — lançamento só com email; o manual 3 fica registrado como adiado em `docs/qa/`* **23/09: fechado** — 1, 2 e 4 ✅; o 3 (Twilio Sandbox) fica fora desta versão por decisão (21/09), com o WhatsApp entregue numa versão futura*
...
- [x] Credenciais PayPal Live em produção; webhook Live cadastrado — *21/09: app Live criado no Developer Dashboard; `PAYPAL_ENV=live` + credenciais nos serviços `launch_os`, `sidekiq` e `backup`; webhook Live `1SX830858W927974S` criado com `rake paypal:webhook_create` (credenciais injetadas via `railway run`, sem passar pelo chat) e redeploy. **Achados:** (1) comprador BR → vendedor BR só em BRL — a tentativa em USD com a conta do Rafael voltou "O vendedor não aceita pagamentos em sua moeda" (pedido `e52d36e3` ficou pending); a preferência "aceitar e converter" não persiste em conta BR porque a conversão é automática. (2) O SDK tinha `currency=USD` fixo → agora vem de `data-currency`; `Product` aceita USD/BRL (#44). (3) **Rotacionar o Secret Live** — ele apareceu numa seleção do editor durante a sessão*
- [x] `TWILIO_ENABLED` conforme aprovação do sender (true só com sender + template aprovados) — *23/09: `TWILIO_ENABLED=false` em produção; sender e template não foram solicitados nesta versão (decisão 21/09)*
...
- [x] C.6 Três criativos (dor / mecanismo / transformação), sem claims absolutos — *22/09: briefing em `docs/conteudo/criativos.md` (copy dos 3 anúncios, formatos, regras de política da Meta, UTMs e prompts). 23/09: artes produzidas (`pain-01` mesa/post-it, `method-01` os 4 passos, `outcome-01` tracker de 21 quadradinhos, corrigido de 30 para 21 na revisão) nos dois formatos (4:5 no feed, 9:16 atribuído por posicionamento em Stories/Reels/Status) e a campanha montada como **rascunho** no Gerenciador*
