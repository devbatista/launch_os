...
- [ ] Serviço `worker` no Railway (mesmo repo, config `railway.worker.json` com `startCommand` do Sidekiq e sem health check, mesmas variáveis; `DATABASE_URL`/`REDIS_URL` como referências aos plugins) — *em andamento 17/09: arquivo criado; falta criar o serviço, apontar o Config File Path no dashboard e confirmar o boot do Sidekiq nos logs*
