...
- [ ] Uptime monitor ativo; backup diário confirmado e **restauração testada**; versionamento do bucket — *UptimeRobot ativo desde 16/09; versionamento do bucket ativo desde a Fase 0. **Decisão 20/09:** `pg_dump` para o bucket em vez do Railway Pro — `Backups::DatabaseDump` + `rake backup:database|list|download`, retenção 30; restauração testada em dev (dump → MinIO → `pg_restore` → contagens iguais). **Achado 20/09:** o Postgres do Railway é **18.6** e a imagem tinha `pg_dump` 17 (recusa servidor mais novo) → `postgresql-client-18` via PGDG no Dockerfile. Falta: criar o serviço cron no Railway (`0 6 * * *`, `bin/rails backup:database` — README) e rodar/restaurar um dump de produção*
