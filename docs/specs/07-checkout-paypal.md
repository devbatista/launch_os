...
- [x] Refund no Sandbox → `refunded`, token revogado, email enviado. *(18/09: refund pela API → webhook via túnel → `MarkRefunded` → `refund_confirmation` no `/letter_opener`, `MessageLog` `sent`)*
