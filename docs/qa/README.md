...
| 5 | Produção: compra real de US$ 14.90 → reembolso → taxas | 🟦 | 21/09 **produção, PayPal Live, em BRL**: produto temporário `live-test` (R$ 5,00) → pedido `085ab158` COMPLETED, webhooks `APPROVED`/`COMPLETED` válidos, email de entrega recebido, 1 download, reembolso pelo painel → `PAYMENT.CAPTURE.REFUNDED` em segundos, token revogado, email de reembolso; taxa R$ 0,84 (4,79% + R$ 0,60). Produto arquivado. **Em USD não dá com conta BR** (regra BR→BR só em BRL). **Decisão 21/09:** a validação em USD será a primeira venda real da campanha — acompanhar de perto (admin, Sentry, `pending_reason`) e calibrar `PAYPAL_FEE_*` pelo breakdown do capture |
